        ...
        async with async_session_maker() as db:
            try:
                ...
                if reference_image_urls and len(reference_image_urls) > 0:
                    reference_boost = (
                        "【参考图：风格强一致 + 内容中等强度参考】\n"
                        "你会收到参考图片（image）。请同时满足：\n"
                        "A) 风格必须强一致（必须遵守）\n"
                        "- 画风：保持参考图整体艺术风格一致（写实/插画/二次元/3D等）\n"
                        "- 质感：保持相近的光影、色调、材质质感、颗粒/锐度、对比度\n"
                        "- 氛围：保持相近的背景氛围与色彩倾向\n"
                        "B) 内容必须“可见参考”（必须遵守，但不锁死人设/服装）\n"
                        "- 必须复用参考图中的【主主体类型/主物体】（例如人物/产品/核心图标/核心道具），让观众一眼能看出与参考图有关联\n"
                        "- 必须至少保留以下 3 类中的任意 2 类作为视觉锚点：\n"
                        "  1) 主配色/配色比例（例如主色+点缀色）\n"
                        "  2) 标志性道具/符号/图标/文字元素（若参考图有明显文字/图标，尽量保留其存在感）\n"
                        "  3) 背景关键元素或可辨识的场景特征（例如室内/医院/讲解板/按钮UI等）\n"
                        "- 允许变化：人物可换装/换姿势/换镜头；构图可调整；允许新增/删减次要元素\n"
                        "- 冲突处理：若文本要求与参考图内容冲突，优先保留参考图的关键视觉锚点，再用文本做补充\n"
                        "在满足 A 与 B 的前提下，按以下文本要求生成：\n"
                    )
